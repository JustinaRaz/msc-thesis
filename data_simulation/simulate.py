...
def input_parse():
    ...
    parser.add_argument(
        "--prompt_language",
        help="Prompt language to use in the experiment.",
        type=str,
        required=True,
    )
    parser.add_argument(
...
def simulate_conversation(
    ...
    tutor_history = ChatHistory(
        messages=[
            ...
            ),  # Fixed first message to the tutor LLM
        ]
    )
...
def main():
    ...
    for n in range(n_runs):
        ...
        save_dir = Path(
            "data_simulation/output/reproducibility"
        ...
        if args.student_constrain:
            save_dir = Path(f"data_simulation/output/reproducibility/{model.model_id.replace("/", "--")}/student_constrained"
                / f"{args.prompt_language}"
                / args.prompt_cefr_level
            )
